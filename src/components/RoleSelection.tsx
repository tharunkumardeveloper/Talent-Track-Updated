      ...
      <div className="max-w-4xl w-full">
        ...
        <div className="flex flex-col space-y-3 md:grid md:grid-cols-3 md:gap-6 md:space-y-0">
          {roles.map((role) => (
            <div
              key={role.id}
              className="glass glass-hover rounded-lg md:rounded-3xl p-3 md:p-8 border border-glass-border/50 text-center group cursor-pointer w-full"
              onClick={() => login(role.id)}
            >
              <div className={`w-10 h-10 md:w-20 md:h-20 mx-auto mb-2 md:mb-6 rounded-lg md:rounded-2xl ${role.bgColor} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
